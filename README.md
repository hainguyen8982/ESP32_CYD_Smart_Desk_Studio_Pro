...
| Hạng mục / Thông số | Tần suất cập nhật | Nguồn dữ liệu / Phương thức |
| :--- | :--- | :--- |
| **PC Hardware Metrics** (CPU, RAM, GPU, Net, Disks) | **1 giây / lần (Realtime)** | Nạp từ PC App (`Smart Desk Studio`) qua Cáp USB Serial (COM) hoặc WiFi HTTP |
...
| **Thời Tiết & Dự Báo 3 Ngày** | **15 phút / lần** | API Open-Meteo (`api.open-meteo.com`) (Tự động theo tọa độ Tỉnh/Thành VN) |
| **Đồng Hồ & Lịch Âm Dương** | **100ms (Nội bộ)** | NTP Time Server (`vn.pool.ntp.org`) & Thuật toán Âm lịch VN |
| **Cấu hình Ngoại tệ, Theme, Calibration** | **Tức thì & Lưu NVS Flash** | Bộ nhớ Flash vĩnh viễn NVS (`Preferences`) & Đồng bộ UDP/Serial 0ms |
...
### 2. Dịch Vụ Giám Sát PC (Smart Desk Studio)
- **File thực thi `.exe` thương mại độc lập (chạy ẩn, không cửa sổ Terminal đen)**:  
...
## 📌 Lộ Trình Phát Hành & Đóng Gói App PC (Future Commercial Installer Roadmap)

Kế hoạch đóng gói ứng dụng PC chuyên nghiệp cho người dùng cuối (End-users):
- 📦 **Bộ cài đặt chuyên nghiệp (`Setup_Smart_Desk_Studio.exe`)**: Xây dựng kịch bản cài đặt tự động qua **Inno Setup / NSIS**.
