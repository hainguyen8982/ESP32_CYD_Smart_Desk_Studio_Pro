...
void NetworkManager::setupWebRoutes() {
    ...
    server.on("/", HTTP_GET, [this](AsyncWebServerRequest *request) {
        String html = "<!DOCTYPE html><html lang='vi'><head><meta charset='UTF-8'>"
                      ...
                      ".theme-btn{background:#030712;border:1px solid var(--border);border-radius:10px;padding:10px 6px;color:#fff;font-size:12px;font-weight:700;display:flex;flex-direction:column;align-items:center;gap:6px;cursor:pointer;transition:0.2s}"
                      ".theme-btn.active{border-color:var(--accent);box-shadow:0 0 10px rgba(57,255,20,0.3)}"
                      ".theme-btn:active{transform:scale(0.96)}"
                      ".theme-swatch{display:flex;gap:3px;align-items:center;justify-content:center;background:#1e293b;padding:4px 6px;border-radius:10px;border:1px solid #334155}"
                      ".theme-swatch span{width:10px;height:10px;border-radius:50%;display:inline-block;border:1px solid rgba(255,255,255,0.3)}"
                      ...
                      "<div class='card'>"
                      "<div class='card-title'>🎨 Theme Presets (Phối Màu Trực Quan)</div>"
                      "<div class='grid-3'>"
                      "<button class='theme-btn' id='th_ocean_dark' onclick=\"setTheme('ocean_dark')\"><div class='theme-swatch'><span style='background:#0f172a'></span><span style='background:#38bdf8'></span><span style='background:#3b82f6'></span></div><span>Ocean</span></button>"
                      "<button class='theme-btn' id='th_cyberpunk' onclick=\"setTheme('cyberpunk')\"><div class='theme-swatch'><span style='background:#18042c'></span><span style='background:#ff00cc'></span><span style='background:#00f0ff'></span></div><span>Cyberpunk</span></button>"
                      "<button class='theme-btn' id='th_forest' onclick=\"setTheme('forest')\"><div class='theme-swatch'><span style='background:#062c19'></span><span style='background:#10b981'></span><span style='background:#a3e635'></span></div><span>Forest</span></button>"
                      "<button class='theme-btn' id='th_cherry' onclick=\"setTheme('cherry')\"><div class='theme-swatch'><span style='background:#2d0612'></span><span style='background:#f472b6'></span><span style='background:#ff4d4d'></span></div><span>Cherry</span></button>"
                      "<button class='theme-btn' id='th_light_day' onclick=\"setTheme('light_day')\"><div class='theme-swatch'><span style='background:#f8fafc'></span><span style='background:#0284c7'></span><span style='background:#1e293b'></span></div><span>Light Day</span></button>"
                      "<button class='theme-btn' id='th_retro_green' onclick=\"setTheme('retro_green')\"><div class='theme-swatch'><span style='background:#051005'></span><span style='background:#00ff41'></span><span style='background:#52ff78'></span></div><span>Retro</span></button>"
                      "</div></div>"
                      "<div class='card'>"
                      ...
                      "</div>"
                      "<div class='card'>"
                      "<div class='card-title'>💱 Cặp Ngoại Tệ Theo Dõi</div>"
                      "<div style='margin-bottom:8px'><label>Ngoại tệ 1 (Màn chính):</label><select class='select-box' id='cur1'><option value='USD'>USD - Đô la Mỹ</option><option value='EUR'>EUR - Euro</option><option value='JPY'>JPY - Yên Nhật</option><option value='GBP'>GBP - Bảng Anh</option><option value='AUD'>AUD - Đô la Úc</option><option value='SGD'>SGD - Đô la Singapore</option><option value='CNY'>CNY - Nhân dân tệ</option><option value='KRW'>KRW - Won Hàn Quốc</option><option value='CAD'>CAD - Đô la Canada</option><option value='CHF'>CHF - Franc Thụy Sĩ</option></select></div>"
                      "<div style='margin-bottom:8px'><label>Ngoại tệ 2 (Màn phụ):</label><select class='select-box' id='cur2'><option value='EUR'>EUR - Euro</option><option value='USD'>USD - Đô la Mỹ</option><option value='JPY'>JPY - Yên Nhật</option><option value='GBP'>GBP - Bảng Anh</option><option value='AUD'>AUD - Đô la Úc</option><option value='SGD'>SGD - Đô la Singapore</option><option value='CNY'>CNY - Nhân dân tệ</option><option value='KRW'>KRW - Won Hàn Quốc</option><option value='CAD'>CAD - Đô la Canada</option><option value='CHF'>CHF - Franc Thụy Sĩ</option></select></div>"
                      "<button class='btn-primary' onclick='applyFX()'>💱 Áp Dụng Cặp Ngoại Tệ</button>"
                      "</div></div>"
                      "<!-- TAB 1: ALARM & MEMO -->"
                      ...
                      "function applyFX(){var c1=document.getElementById('cur1').value;var c2=document.getElementById('cur2').value;fetch('/api/exchange?cur1='+c1+'&cur2='+c2).then(function(){showToast('💱 Đã đổi cặp ngoại tệ: '+c1+' / '+c2);});}"
                      ...
                      "function syncDeviceState(){fetch('/api/state').then(function(r){return r.json();}).then(function(d){if(d.theme)highlightTheme(d.theme);if(d.city){var s=document.getElementById('citySelect');s.value=d.city;}if(d.cur1){var s1=document.getElementById('cur1');s1.value=d.cur1;if(!s1.value)s1.selectedIndex=0;}if(d.cur2){var s2=document.getElementById('cur2');s2.value=d.cur2;if(!s2.value)s2.selectedIndex=1;}if(d.alarm_h!==undefined)document.getElementById('almH').value=d.alarm_h;if(d.alarm_m!==undefined)document.getElementById('almM').value=d.alarm_m;if(d.alarm_memo!==undefined)document.getElementById('almNote').value=d.alarm_memo;if(d.ip){document.getElementById('ipLbl').innerText=d.ip;}if(d.rssi){document.getElementById('rssiLbl').innerText=d.rssi+' dBm';}}).catch(function(){});}"
